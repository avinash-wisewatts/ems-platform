...
The full list with context is in §16 and in each requirement's `Notes`. Headline unknowns, all `OPEN — PRODUCT DECISION REQUIRED`:

1. First landing page after login.
2. Portfolio-first vs. site-first for multi-site customers.
3. The five headline things on the initial Site Overview.
4. How customers select among multiple sites.
5. Which roles exist in the customer application.
6. How alerts appear (and where they are configured).
7. Which energy-cost capabilities are commercially required.
8. Initial target industries.
9. How much asset detail customers should see.
10. Required reporting formats.
11. Whether Space is primary navigation or drill-down.
12. How functional categories are defined and by whom.
13. What "normal" / baseline performance means, and who sets it.

---

## 13. Product owner's five key questions

`[OPEN]` — recorded verbatim, **OPEN until answered**. These drive the next revision of this set.

1. **When a customer logs in, what should they see first?**
2. **If a customer has multiple sites, should they land at portfolio level or directly on a site?**
3. **What are the five most important things the customer should know about their facility?**
4. **When something looks wrong, how far should the customer be able to investigate?**
   ...
   ```
5. **What should our EMS do substantially better than the ZeroWatt product / reference?**

`[INFERRED]` candidate answer directions (explicitly not decisions):
- Q1: a Site Overview for single-site; a Portfolio Overview for multi-site. `PRODUCT OWNER REVIEW REQUIRED`.
- Q3 shortlist to react to: (a) today's energy vs. expected, (b) current/peak demand vs. contracted, (c) cost-to-date vs. budget, (d) worst-performing space or system, (e) data-quality / connectivity health.
- Q5 candidates: semantic context around every measurement (no raw-tag overload); honest data-quality; a genuinely progressive "why" path backed by the asset-relationship graph; parity-guaranteed energy numbers; independent, fast, role-aware delivery.
