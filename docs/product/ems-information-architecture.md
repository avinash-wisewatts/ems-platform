...
`OPEN — PRODUCT DECISION REQUIRED` (cross-referenced to `ems-product-definition.md` §12 / §13):

- Landing page after login (#1); portfolio-first vs. site-first (#2).
- The five headline metrics on Site Overview (#3).
- Multi-site selection pattern (#4).
- Space as primary nav vs. drill-down (#11).
- How Alerts appear and where they're configured (#6).
- Functional-category definition and how it drives Breakdown/navigation (#12, gap PA-2).
- Which cost capabilities are in scope, if any (#7, gap PA-3).
- How much asset detail customers see (#9).
- Required report formats (#10).
- Default time period per screen family.
- Performance budgets (first-paint / interaction).
