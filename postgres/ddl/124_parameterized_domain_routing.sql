-- 005_parameterized_domain_routing.sql
-- Force affected-event expansion through parameterized LATERAL probes.
-- This prevents PostgreSQL from flattening the small window-event join into a
-- hash join that scans the full normalized_points hypertable. Public route
-- views remain unchanged for compatibility and diagnostics.
BEGIN;
