-- ============================================================================
-- Rollback for migration 226 (Phase 3 -- environment_measurements.space_id +
-- Space resolution in telemetry.load_environment_measurements_incremental).
--
-- Controlled, dependency-checked reversal. NOT run as part of any migration.
-- No CASCADE. Removes only what migration 226 created:
--   * the FK constraint environment_measurements_space_id_fkey
--   * the column telemetry.environment_measurements.space_id
--   * the space_id resolution in the environment loader (by restoring the
--     verbatim pre-226 / migration-207 procedure body)
--
-- Refuses to run if anything outside migration 226 has come to depend on
-- telemetry.environment_measurements.space_id (a view, index, constraint,
-- generated column, or default expression that references it).
--
-- Run manually, inside a single transaction:
--   docker compose exec -T <db> psql -X -v ON_ERROR_STOP=1 -U ems_admin -d <db> \
--     -f postgres/maintenance/226_environment_space_binding_rollback.sql
-- ============================================================================

BEGIN;

-- ----------------------------------------------------------------------------
-- 0. Dependency check. pg_depend / pg_attrdef / pg_constraint / pg_index
--    references to the space_id column, excluding the FK this rollback drops.
-- ----------------------------------------------------------------------------
DO $$
DECLARE
    v_attnum SMALLINT;
...
BEGIN
    ...
    FROM pg_attribute
    WHERE attrelid = 'telemetry.environment_measurements'::regclass
      AND attname  = 'space_id'
      AND NOT attisdropped;

    IF v_attnum IS NULL THEN
        RAISE NOTICE 'Rollback 226: space_id column already absent -- nothing to do for the column/FK.';
        RETURN;
    END IF;

    -- Indexes that include space_id
    IF EXISTS (
        SELECT 1 FROM pg_index i
        WHERE i.indrelid = 'telemetry.environment_measurements'::regclass
          AND v_attnum = ANY (i.indkey)
    ) THEN
        RAISE EXCEPTION 'Rollback 226 aborted: an index on telemetry.environment_measurements references space_id. Migration 226 created no index; investigate before rolling back.';
    END IF;

    -- Constraints other than the FK this rollback removes
    IF EXISTS (
        SELECT 1 FROM pg_constraint c
        WHERE c.conrelid = 'telemetry.environment_measurements'::regclass
          AND v_attnum = ANY (c.conkey)
          AND c.conname <> 'environment_measurements_space_id_fkey'
    ) THEN
        RAISE EXCEPTION 'Rollback 226 aborted: a constraint other than environment_measurements_space_id_fkey references space_id.';
    END IF;

    -- Any other object depending on the column (e.g. a view)
    SELECT string_agg(DISTINCT format('%s (%s)', d.refobjid::regclass, d.deptype), ', ')
      INTO v_refs
    FROM pg_depend d
    WHERE d.classid = 'pg_class'::regclass
      AND d.objid   = 'telemetry.environment_measurements'::regclass
      AND d.objsubid = v_attnum
      AND d.deptype <> 'a';   -- 'a' = auto (the column's own dependency)

    IF v_refs IS NOT NULL AND v_refs <> '' THEN
        RAISE EXCEPTION 'Rollback 226 aborted: objects depend on telemetry.environment_measurements.space_id: %', v_refs;
    END IF;
END;
$$;
...
-- ----------------------------------------------------------------------------
-- 1. Restore the verbatim pre-226 (migration 207) environment loader body.
-- ----------------------------------------------------------------------------
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Rollback 226 postcondition failed: space_id column still present.';
    END IF;

    v_body := pg_get_functiondef('telemetry.load_environment_measurements_incremental(interval,interval)'::regprocedure);
    IF position('space_points' IN v_body) <> 0 THEN
        RAISE EXCEPTION 'Rollback 226 postcondition failed: the environment loader body still references space_points.';
    END IF;
END;
$$;
