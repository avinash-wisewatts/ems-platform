-- ============================================================================
-- Rollback for Migration 227 (Phase 4 -- declarative parameter routing foundation)
--
-- Reverses postgres/migrations/227_parameter_routing_foundation.sql:
--   1. Restores telemetry.load_environment_measurements_incremental(interval,
--      interval) to the VERBATIM deployed migration-226 body (byte-for-byte,
--      comments included) -- i.e. the pre-Phase-4 environment routing
--      implementation.
--   2. Drops config.parameter_routing (no CASCADE), after a dependency check.
--
-- Safety:
--   * Dependency-checked: refuses to drop config.parameter_routing if any
--     object outside the table's own constraints/indexes/defaults/toast
--     depends on it (pg_depend). No CASCADE anywhere.
--   * Safe if migration 227 was never applied: the loader restore is an
--     idempotent CREATE OR REPLACE to the body that is already deployed, and
--     the table drop is guarded with IF EXISTS.
--   * Touches nothing else: no energy object, no job, no Space binding, no
--     telemetry data, no metadata.space_points / metadata.spaces.
--
-- Not run automatically. Apply manually against the target database only when
-- a Phase 4 rollback is explicitly authorised.
-- ============================================================================


-- ----------------------------------------------------------------------------
-- 1. Dependency check -- fail loudly rather than CASCADE.
-- ----------------------------------------------------------------------------
...
BEGIN
    IF to_regclass('config.parameter_routing') IS NULL THEN
        RAISE NOTICE 'Rollback 227: config.parameter_routing is already absent -- nothing to drop.';
        RETURN;
    END IF;

    SELECT string_agg(DISTINCT format('%s %s', d.deptype, d.objid::regclass), ', ')
    INTO v_blockers
    FROM pg_depend d
    WHERE d.refobjid = 'config.parameter_routing'::regclass
      AND d.deptype NOT IN ('a', 'i')          -- auto / internal (own constraints, indexes, defaults)
      AND d.classid <> 'pg_constraint'::regclass
      AND d.classid <> 'pg_attrdef'::regclass
      AND NOT (d.classid = 'pg_class'::regclass       -- own indexes / toast
               AND d.objid::regclass::text LIKE 'config.%parameter_routing%');

    IF v_blockers IS NOT NULL THEN
        RAISE EXCEPTION 'Rollback 227 aborted: unexpected dependencies on config.parameter_routing: %', v_blockers;
    END IF;
END;
$rollback_guard$;
...
-- ----------------------------------------------------------------------------
-- 2. Restore the VERBATIM deployed migration-226 environment loader body.
--    Reproduced byte-for-byte from
--    postgres/migrations/226_environment_space_binding.sql (lines 111-413),
--    comments included -- so pg_get_functiondef after this rollback matches
--    the pre-Phase-4 definition exactly.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 3. Drop the routing table (no CASCADE; guarded).
-- ----------------------------------------------------------------------------

DROP TABLE IF EXISTS config.parameter_routing;
...
DECLARE
    v_env_body TEXT;
BEGIN
    IF to_regclass('config.parameter_routing') IS NOT NULL THEN
        RAISE EXCEPTION 'Rollback 227 postcondition failed: config.parameter_routing still exists.';
    END IF;

    v_env_body := pg_get_functiondef('telemetry.load_environment_measurements_incremental(interval,interval)'::regprocedure);

    -- the Phase-4 provenance sentence must be gone
    IF position('Phase 4 (migration 227)' IN v_env_body) <> 0 THEN
        RAISE EXCEPTION 'Rollback 227 postcondition failed: the environment loader still carries the Phase 4 COMMENT sentence.';
    END IF;
    -- but the migration-226 Space resolution and NULL quality_code must remain
    IF position('metadata.space_points' IN v_env_body) = 0 THEN
        RAISE EXCEPTION 'Rollback 227 postcondition failed: the restored environment loader lost its Space resolution sub-select.';
    END IF;
    IF position('NULL::SMALLINT AS quality_code' IN v_env_body) = 0 THEN
        RAISE EXCEPTION 'Rollback 227 postcondition failed: the restored environment loader no longer writes quality_code = NULL.';
    END IF;

    -- energy untouched
    IF position('config.parameter_routing' IN
                pg_get_functiondef('telemetry.load_energy_measurements_incremental(interval,interval)'::regprocedure)) <> 0 THEN
        RAISE EXCEPTION 'Rollback 227 postcondition failed: the energy loader references config.parameter_routing.';
    END IF;
END;
$rollback_post$;
