...
-- ----------------------------------------------------------------------------
-- 0. Sequence-resilience guard -- MUST be the first statement in this
--    transaction, before ANY operation that can allocate a TimescaleDB
--    background-job id: add_retention_policy() in section 1 (it calls add_job
--    internally), and the add_job() loop in section 9.
--
...
--    (observed on staging: seq last_value 1113/1114 vs max(id) 1114, failing at
--    add_retention_policy('analytics.pipeline_reconciliation_log', ...)).
--
--    Advance the sequence to at least the current max id, using GREATEST so a
--    sequence that is legitimately ahead (e.g. after delete_job) is never moved
--    backwards. add_job() allocates via nextval(); setval(<v>, true) sets
--    is_called=true so the next nextval() returns <v>+1 -> strictly greater
--    than every existing id (verified empirically against add_job). Idempotent,
--    and a plain no-op wherever the sequence is already consistent. Guarded by
--    to_regclass so a future TimescaleDB that renames/removes the sequence just
--    skips this.
-- ----------------------------------------------------------------------------
DO $seqfix$
BEGIN
