-- ============================================================================
-- Migration 200
-- Fix analytics.get_canonical_energy_read() raising an exception for a
-- legitimate historical range that simply predates any capture policy.
--
-- Root cause diagnosis (2026-08-25, live investigation): the Asset
-- Dashboard's "Energy Consumption Trend" panel calls
-- analytics.get_canonical_energy_read(..., 'native', 'strict'). For a
-- selected range whose start (p_from) falls before the earliest capture
-- policy this platform has ever recorded for a site (site-specific or the
-- global site_id IS NULL default), telemetry.resolve_site_capture_bucket
-- correctly returns policy_id = NULL, and the function raises:
--
--   ERROR: no capture policy resolvable for site % at %
--
-- This is a DIFFERENT condition from the one migration 191/192 already
-- fixed (a false-positive mid-range policy-identity comparison). This one
-- fires at the very first policy resolution, for p_from itself, before any
-- telemetry table is ever read -- so a "select last 90 days" click on a
-- recently-commissioned site currently produces a database error instead
-- of a chart that simply starts where the site's real data begins.
--
-- The distinction this migration draws, precisely:
--   - p_from before the EARLIEST capture policy this site has ever had
--     (site-specific or global default) means categorically no policy
--     could ever have covered that instant -- no telemetry could exist
--     there either. The correct answer is empty data for that portion,
--     not an error.
--   - p_from at or after that earliest instant, but resolution still
--     fails, means there is a genuine gap between policies during a
--     period the site SHOULD have been configured -- a real
--     configuration inconsistency. This case is deliberately left
--     raising exactly as before; it is not touched by this migration.
--
-- Fix: when policy resolution at p_from fails, resolve the earliest
-- applicable effective_from for the site. If the entire requested range
-- predates it, return zero rows. If the range straddles it, clip the
-- effective read window forward to that instant (contributing zero rows
-- for the pre-policy portion) and proceed exactly as before for the rest.
-- If no applicable policy exists at all, or p_from is already inside the
-- configured era and still fails to resolve, the original exception is
-- preserved unchanged.
--
-- No schema/index change. No change to the migration-191/192
-- boundary-crossing check, the resolution-tier routing logic, or any
-- other function. Idempotent: safe to run more than once (CREATE OR
-- REPLACE FUNCTION).
-- ============================================================================
