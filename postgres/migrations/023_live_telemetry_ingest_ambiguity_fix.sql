...
BEGIN
    IF p_payload IS NULL
       OR jsonb_typeof(p_payload -> 'rtdata') <> 'array' THEN
        RETURN;
    END IF;
    ...
    (
        SELECT
            ...
            CASE
                ...
                THEN to_timestamp(
                    (elem.value ->> 'ts')::DOUBLE PRECISION
                )
                ELSE p_received_at
            END AS event_time
    ...
    (
        ...
        ON CONFLICT
        ON CONSTRAINT device_live_point_state_pkey
        DO UPDATE
        ...
        WHERE
            EXCLUDED.event_time
                > telemetry.device_live_point_state.event_time
            OR (
                EXCLUDED.event_time
                    = telemetry.device_live_point_state.event_time
                AND EXCLUDED.received_at
                    >= telemetry.device_live_point_state.received_at
            )

        RETURNING
            telemetry.device_live_point_state.device_id
    )
...
COMMENT ON FUNCTION telemetry.ingest_live_rtdata(
    TEXT,
    JSONB,
    TIMESTAMPTZ
) IS
