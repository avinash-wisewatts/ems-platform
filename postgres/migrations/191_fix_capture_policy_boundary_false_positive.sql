-- ============================================================================
-- Migration 191
-- Fix the "Datasource error" on the Asset Dashboard's Total Energy tile and
-- Energy Consumption Trend chart.
--
-- Root cause diagnosis (this was NOT caused by migrations 187/188/189's
-- sector/sub-sector taxonomy work -- verified directly: both underlying
-- functions still have the exact signatures the dashboard panels call, and
-- analytics.v_grafana_assets/v_grafana_sites recreated in migration 188
-- resolve correctly):
--
--   Both panels ultimately call analytics.get_canonical_energy_read()
--   (get_grafana_asset_energy_intervals wraps it; the Energy Consumption
--   Trend chart calls it directly). That function refuses any [p_from,
--   p_to) range that crosses what it believes is a genuine change in the
--   site's telemetry capture policy -- correct behavior in principle, since
--   silently applying one interval's semantics across a policy change would
--   corrupt the read.
--
--   The bug: the crossing check compares raw config.telemetry_capture_
--   policies.id (policy row identity) at each candidate boundary instant,
--   not the *actual resolved capture parameters*. But
--   admin.set_site_telemetry_capture_interval ->
--   config.set_site_telemetry_capture_policy always closes the currently
--   active row and inserts a brand-new one on every call -- with no check
--   for whether the requested interval/tolerance actually differ from what
--   is already in effect. Saving a site's settings from the admin portal
--   calls this unconditionally (app/src/location_management_service.py,
--   _set_site_capture_interval, invoked from both site create and site
--   edit), so every save mints a new policy row even when nothing
--   meaningfully changed.
--
--   Live evidence at Meenaxy Pharma / Unit 2 (site
--   f454dc96-0673-4631-8421-8f75a4082a91): 6 policy rows since 2026-08-04,
--   every one with the identical capture_interval_seconds = 60. Each row
--   boundary is nonetheless flagged as a "genuine" policy change by the
--   current policy_id comparison, so any dashboard time window spanning one
--   of those save timestamps -- which, for a window that includes "now",
--   is essentially any window after the most recent site save -- trips the
--   RAISE EXCEPTION and Grafana reports it as a datasource error.
--
-- Fix (two parts, defense in depth):
--   1. analytics.get_canonical_energy_read: compare the actual resolved
--      capture_interval_seconds / late_arrival_tolerance_seconds at each
--      boundary instant, not policy_id. This is what the function's own
--      existing comment already claims it does ("only a real change in
--      what resolves triggers rejection") -- the comparison just wasn't
--      checking the right thing. This alone fixes every existing
--      false-positive boundary retroactively; no historical policy data is
--      modified.
--   2. config.set_site_telemetry_capture_policy: no-op (return the existing
--      row's id without writing) when the requested interval and tolerance
--      already match the currently active policy, so routine site saves
--      stop minting spurious policy rows going forward.
--
...
DECLARE
    ...
    v_boundary_late_arrival_tolerance_seconds INTEGER;
...
BEGIN
    ...
    --
    -- Compares the RESOLVED capture parameters (capture_interval_seconds,
    -- late_arrival_tolerance_seconds), not the policy row id. A new policy
    -- row that resolves to the same actual parameters as the one it
    -- replaced (e.g. an unrelated site-settings save that re-submits the
    -- same telemetry interval) is not a genuine change in what this
    -- function's math depends on, so it must not trip this guard --
    -- policy_id comparison was doing exactly that, rejecting reads across
    -- every such no-op save. This is what the surrounding design already
    -- intended ("only a real change in what resolves triggers rejection");
    -- the comparison itself was checking the wrong thing.
    -- ------------------------------------------------------------------
    ...
    LOOP
        SELECT
            ...
            b.late_arrival_tolerance_seconds
        INTO
            v_boundary_capture_interval_seconds,
            v_boundary_late_arrival_tolerance_seconds
        FROM telemetry.resolve_site_capture_bucket(v_site_id, v_boundary_time) AS b;

        IF v_boundary_capture_interval_seconds IS DISTINCT FROM v_capture_interval_seconds
            OR v_boundary_late_arrival_tolerance_seconds IS DISTINCT FROM v_late_arrival_tolerance_seconds
        THEN
            RAISE EXCEPTION
...
-- ----------------------------------------------------------------------------
-- Prevent future spurious policy rows: no-op when the requested capture
-- interval and late-arrival tolerance already match the currently active
-- policy for the site.
-- ----------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION config.set_site_telemetry_capture_policy(p_site_id uuid, p_capture_interval_seconds integer, p_effective_from timestamp with time zone DEFAULT clock_timestamp(), p_late_arrival_tolerance_seconds integer DEFAULT 900)
 RETURNS bigint
 LANGUAGE plpgsql
AS $function$
DECLARE
    v_policy_id BIGINT;
    v_unchanged_policy_id BIGINT;
BEGIN
    ...
    -- No-op: the currently active policy already resolves to the requested
    -- parameters, so opening a new row would only fracture policy history
    -- without changing anything a consumer can observe.
    SELECT id
    INTO v_unchanged_policy_id
    FROM config.telemetry_capture_policies
    WHERE site_id = p_site_id
      AND is_enabled
      AND effective_to IS NULL
      AND capture_interval_seconds = p_capture_interval_seconds
      AND late_arrival_tolerance_seconds = p_late_arrival_tolerance_seconds
    LIMIT 1;

    IF v_unchanged_policy_id IS NOT NULL THEN
        RETURN v_unchanged_policy_id;
    END IF;
