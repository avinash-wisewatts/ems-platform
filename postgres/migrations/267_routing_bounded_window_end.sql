-- ============================================================================
-- Migration 267
-- Routing window-end lookup: bound max(platform_received_at) to recent
-- event_time in BOTH routing loaders so it stops decompressing historical
-- telemetry.normalized_points chunks on every run.
--
-- WHY
--   Both telemetry.load_energy_measurements_incremental (job 1001) and
--   telemetry.load_environment_measurements_incremental (job 1012) derived
--   their window end on every run with
--       SELECT max(platform_received_at) FROM telemetry.normalized_points
--       WHERE platform_received_at IS NOT NULL;
--   normalized_points is partitioned on event_time and compressed after
--   1 day, and compressed chunks have no index on platform_received_at, so
--   this decompressed and sorted every compressed chunk. One authorized
--   EXPLAIN (ANALYZE, BUFFERS) on staging (2026-09-24): 205.3 s, 71.38M rows
--   decompressed, ~96% of the time in the four decompression scans, while
--   the answer always comes from the recent uncompressed chunks.
--
-- WHAT (only this; everything else in both bodies is byte-for-byte unchanged,
-- proven by the postconditions)
--       SELECT max(platform_received_at) INTO v_window_end
--       FROM telemetry.normalized_points
--       WHERE platform_received_at IS NOT NULL
--         AND event_time >= now() - INTERVAL '1 day';
--       IF v_window_end IS NULL THEN            -- rare liveness fallback
--         SELECT max(platform_received_at) INTO v_window_end
--         FROM telemetry.normalized_points
--         WHERE platform_received_at IS NOT NULL;
--       END IF;
--   The subsequent logic is unchanged: NULL -> NO_SOURCE_DATA, the
--   checkpoint + p_max_window (2 h) cap, the effective 1-minute overlap,
--   point selection, re-fetch, upsert, checkpoint write, and the
--   EXCEPTION/rollback contract.
--
-- SEMANTICS
--   * The bounded max is taken over a subset of rows, so it is never above
--     the old global max. It is equal whenever the most recently received
--     row has an event_time in the last day (normal ingestion). Otherwise it
--     is lower, which only delays routing: a row with a fresher
--     platform_received_at stays after window_end and is selected by a later
--     window. No row can be skipped by this change.
--   * The 1-day bound must stay <= the normalized_points compress_after
--     (1 day, verified below) so every compressed chunk (range_end <
--     now - compress_after at compression time) is excluded.
--   * NOT used: pipeline_state('normalized_points').last_received_at. That
--     checkpoint is the raw-receipt frontier the normalizer considered; it
--     runs ahead of normalized rows while samples wait for bucket
--     finalization, and using it as window_end can permanently skip them.
--   * Unchanged, out of scope: failed-message recovery (recovered rows carry
--     their original received_at and are not routed, as before), the 1-minute
--     overlap vs 900 s tolerance question, job 1000, jobs 1001/1012.
--
-- SOURCES
--   Energy body: migration 215 (identical to the live staging body,
--   CR-normalized md5 d7adba7148a20e523c905f09cff52e8f) with only the block above replaced.
--   Environment body: the offline generator's committed artifact
--   scripts/codegen/generated/load_environment_measurements_incremental.generated.sql,
--   regenerated from the updated template (previous live CR-normalized md5
--   b9db05675b7ae65dae5637b521a6a60b).
-- ============================================================================


-- ----------------------------------------------------------------------------
-- 1. Preconditions: compression boundary and exact source bodies.
-- ----------------------------------------------------------------------------
DO $pre$
DECLARE
...
BEGIN
    ...
    INTO v_compress_after
    FROM timescaledb_information.jobs AS j
    WHERE j.hypertable_schema = 'telemetry'
      AND j.hypertable_name = 'normalized_points'
      AND j.proc_name = 'policy_compression';

    IF v_compress_after IS NULL OR v_compress_after < INTERVAL '1 day' THEN
        RAISE EXCEPTION 'Migration 267 precondition failed: normalized_points compress_after is % (the 1-day routing bound must be <= compress_after).', v_compress_after;
    END IF;

    IF (SELECT md5(replace(prosrc, E'\r', '')) FROM pg_proc
        WHERE oid = 'telemetry.load_energy_measurements_incremental(interval,interval)'::regprocedure)
       IS DISTINCT FROM 'd7adba7148a20e523c905f09cff52e8f' THEN
        RAISE EXCEPTION 'Migration 267 precondition failed: load_energy_measurements_incremental differs from the expected (migration 215) body.';
    END IF;

    IF (SELECT md5(replace(prosrc, E'\r', '')) FROM pg_proc
        WHERE oid = 'telemetry.load_environment_measurements_incremental(interval,interval)'::regprocedure)
       IS DISTINCT FROM 'b9db05675b7ae65dae5637b521a6a60b' THEN
        RAISE EXCEPTION 'Migration 267 precondition failed: load_environment_measurements_incremental differs from the expected (migration 228) body.';
    END IF;
END
$pre$;
...
-- ----------------------------------------------------------------------------
-- 2. Energy loader: migration 215 body with only the window-end block replaced.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 3. Environment loader: the regenerated offline-generator artifact
--    (scripts/codegen/generated/load_environment_measurements_incremental.generated.sql),
--    verbatim.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4. Postconditions: each body contains the bounded block exactly once, and
--    reverting that block reproduces the previous body byte-for-byte.
-- ----------------------------------------------------------------------------
DO $post$
DECLARE
    v_energy TEXT;
    v_env    TEXT;
BEGIN
    SELECT replace(prosrc, E'\r', '') INTO v_energy FROM pg_proc
    WHERE oid = 'telemetry.load_energy_measurements_incremental(interval,interval)'::regprocedure;
    SELECT replace(prosrc, E'\r', '') INTO v_env FROM pg_proc
    WHERE oid = 'telemetry.load_environment_measurements_incremental(interval,interval)'::regprocedure;

    IF (length(v_energy) - length(replace(v_energy, '  -- Migration 267: bound the window-end lookup to recent event_time so it only
  -- reads uncompressed chunks (normalized_points compress_after = 1 day). The
  -- bounded max can only be lower than the global max, which delays -- never
  -- skips -- rows; the global lookup remains the fallback when no row has a
  -- recent event_time.
  ...
  IF v_window_end IS NULL THEN
    ...
    FROM telemetry.normalized_points
    WHERE platform_received_at IS NOT NULL;
  END IF;', ''))) / length('  -- Migration 267: bound the window-end lookup to recent event_time so it only
  -- reads uncompressed chunks (normalized_points compress_after = 1 day). The
  -- bounded max can only be lower than the global max, which delays -- never
  -- skips -- rows; the global lookup remains the fallback when no row has a
  -- recent event_time.
  ...
  IF v_window_end IS NULL THEN
    ...
    WHERE platform_received_at IS NOT NULL;
  END IF;') <> 1 THEN
        RAISE EXCEPTION 'Migration 267 postcondition failed: energy loader does not contain the bounded window-end block exactly once.';
    END IF;
    IF (length(v_env) - length(replace(v_env, '    -- Migration 267: bound the window-end lookup to recent event_time so it only
    -- reads uncompressed chunks (normalized_points compress_after = 1 day). The
    -- bounded max can only be lower than the global max, which delays -- never
    -- skips -- rows; the global lookup remains the fallback when no row has a
    -- recent event_time.
    ...
    IF v_window_end IS NULL THEN
      ...
      WHERE platform_received_at IS NOT NULL;
    END IF;') <> 1 THEN
        RAISE EXCEPTION 'Migration 267 postcondition failed: environment loader does not contain the bounded window-end block exactly once.';
    END IF;

    IF md5(replace(v_energy, '  -- Migration 267: bound the window-end lookup to recent event_time so it only
  -- reads uncompressed chunks (normalized_points compress_after = 1 day). The
  -- bounded max can only be lower than the global max, which delays -- never
  -- skips -- rows; the global lookup remains the fallback when no row has a
  -- recent event_time.
  ...
  IF v_window_end IS NULL THEN
    ...
    FROM telemetry.normalized_points
    WHERE platform_received_at IS NOT NULL;
  END IF;', '  SELECT max(platform_received_at) INTO v_window_end
  FROM telemetry.normalized_points
  WHERE platform_received_at IS NOT NULL;')) <> 'd7adba7148a20e523c905f09cff52e8f' THEN
        RAISE EXCEPTION 'Migration 267 postcondition failed: energy loader changed outside the window-end block.';
    END IF;
    IF md5(replace(v_env, '    -- Migration 267: bound the window-end lookup to recent event_time so it only
    -- reads uncompressed chunks (normalized_points compress_after = 1 day). The
    -- bounded max can only be lower than the global max, which delays -- never
    -- skips -- rows; the global lookup remains the fallback when no row has a
    -- recent event_time.
    ...
    END IF;', '    SELECT max(platform_received_at) INTO v_window_end
    FROM telemetry.normalized_points
    WHERE platform_received_at IS NOT NULL;')) <> 'b9db05675b7ae65dae5637b521a6a60b' THEN
        RAISE EXCEPTION 'Migration 267 postcondition failed: environment loader changed outside the window-end block.';
    END IF;

    RAISE NOTICE 'Migration 267: all postconditions passed (both routing loaders use the event_time-bounded window-end lookup with the global fallback; nothing else changed).';
END
$post$;
