...
gateway_readiness AS (
    SELECT
        ...
        CASE
            ...
            WHEN nullif(btrim(g.external_id), '') IS NULL THEN 'BLOCKED'
            ELSE 'READY'
        END AS commissioning_status,
        (
...
'Declarative commissioning readiness for assets, gateways, and devices. Portal actions must consume this contract rather than reproduce readiness rules.';

DROP FUNCTION IF EXISTS admin.list_accessible_commissioning_readiness(BIGINT, TEXT);
CREATE FUNCTION admin.list_accessible_commissioning_readiness(
...
SELECT
    ...
    readiness.site_id,
    readiness.entity_name,
    readiness.lifecycle_status,
    readiness.commissioning_status,
    readiness.is_ready,
    readiness.blocking_reason_codes,
    readiness.warning_reason_codes
FROM analytics.v_commissioning_readiness readiness
WHERE admin.portal_user_can_access_site(p_actor_portal_user_id, readiness.site_id)
  AND (p_entity_type IS NULL OR readiness.entity_type = upper(btrim(p_entity_type)))
ORDER BY readiness.entity_type, readiness.entity_name;
$function$;

CREATE OR REPLACE FUNCTION admin.commission_asset(
    p_actor_portal_user_id BIGINT,
    p_asset_id UUID
)
RETURNS JSONB
...
BEGIN
    ...
    IF NOT FOUND OR NOT admin.portal_user_has_permission(p_actor_portal_user_id, 'asset.manage') THEN
        RAISE EXCEPTION 'Portal actor is not authorized to commission assets.' USING ERRCODE = '42501';
    END IF;

    SELECT site_id, lifecycle_status, metering_requirement
    INTO v_site_id, v_old_lifecycle, v_metering_requirement
    ...
    IF v_old_lifecycle = 'DECOMMISSIONED' THEN
        RAISE EXCEPTION 'A decommissioned asset cannot be commissioned.' USING ERRCODE = '23514';
    END IF;
    ...
    IF NOT FOUND OR NOT COALESCE(v_readiness, FALSE) THEN
        RAISE EXCEPTION 'Asset commissioning is blocked: %', array_to_string(COALESCE(v_blockers, ARRAY['READINESS_UNAVAILABLE']::text[]), ', ')
            USING ERRCODE = '23514';
    END IF;
...
ALTER FUNCTION admin.list_accessible_commissioning_readiness(BIGINT, TEXT) OWNER TO ems_admin;
ALTER FUNCTION admin.commission_asset(BIGINT, UUID) OWNER TO ems_admin;
REVOKE ALL ON FUNCTION admin.list_accessible_commissioning_readiness(BIGINT, TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION admin.commission_asset(BIGINT, UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION admin.list_accessible_commissioning_readiness(BIGINT, TEXT) TO ems_app;
GRANT EXECUTE ON FUNCTION admin.commission_asset(BIGINT, UUID) TO ems_app;
